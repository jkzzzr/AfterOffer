...
import my.decoration.Decoration;
...
public class Ndeval {
	...
	public void computeERR(rList rl){
		...
		for (int i = 0; i < DEPTH; i++){
	      idealIdeal[i] = idealIdealGain/(i + 1);
	      idealIdealGain *= (1.0 - alpha);
	    }
	...
	public ArrayList<rList> processQrels (String fileName) throws NumberFormatException, IOException{
		...
		while((templine=reader.readLine())!=null){
			...
			if(rel>0){
				qrels.add(new qrel(docid, topic, subtopic, 1));
			}	
		}
		reader.close();
	...
	public ArrayList<rList> processRun (File input,boolean isbaseline) throws Exception{
		...
		for (int i = 1; i < r.size(); i++)
			if (r.get(i).topic == r.get(i-1).topic && r.get(i).rank == r.get(i-1).rank)
				throw new Exception("duplicate rank "+r.get(i).rank+":"+r.get(i-1).rank+" for topic "+r.get(i).topic+
						" in run file "+input.getName()+" line:"+i);
...
//				break;
	...
	public double[] statisticalAverage(double[][] result){
		...
		try {
			writePCEval(result, 3, 12,"F:\\Fusion\\final\\choose\\123");
		} catch (IOException e) {
	...
	public void writePCEval(double[][] result,int index1, int index2, String outputPath) throws IOException{
		BufferedWriter bw = new BufferedWriter(new FileWriter(outputPath, true));
		bw.write("erria结果");
		
		for(int i=0;i<result.length;i++){
			bw.write(result[i][index1]+"\n");
		}
		bw.write("\n");
		bw.write("andcg结果");
		
		for(int i=0;i<result.length;i++){
			bw.write(result[i][index2]+"\n");
		}
		bw.close();
	}
	...
	/*	String line=runstring+",amean";	
		...
		System.out.println(line);*/
		System.out.println(runstring+"\t"+average[2]+"\t"+average[11]);
	}
	 ...
	 * @param rl
	 * @param bl
	 * @return average[2]==>>ERR-IA@20    average[11]==>>alpha-nDCG@20
	 */
	public double[] returnMetrics(ArrayList<rList> rl,ArrayList<rList> bl){
		double[][] result=statisticalAll(rl, bl);	
		double[] average=statisticalAverage(result);
	/*	System.out.print("runid,topic");
		System.out.print (",ERR-IA@5,ERR-IA@10,ERR-IA@20");
	...
	public String evaluate(File input,String qrelfile) throws Exception{
		ArrayList<rList> qrl=processQrels(qrelfile);	
		ArrayList<rList> rrl=processRun(input, false);
		applyQrels(qrl, rrl);
		double[] mean=statisticalAverage(statisticalAll(rrl, null));
//		outputMetrics(rrl, null);
		String r=input.getName()+","+mean[2]+","+mean[11]+","+mean[14]+","+mean[15];
		System.out.println(r);
		return r+"\n";
	}
	public void evaluateALL(File input, String qrelfile) throws Exception, IOException{
		ArrayList<rList> qrl=processQrels(qrelfile);	
		ArrayList<rList> rrl=processRun(input, false);
		applyQrels(qrl, rrl);
		outputMetrics(rrl, null);
	 ...
	 * @throws IOException
	 */
	public double[] RETURNevaluateALL(File input, String qrelfile) throws Exception, IOException{
		ArrayList<rList> qrl=processQrels(qrelfile);	
		ArrayList<rList> rrl=processRun(input, false);
		applyQrels(qrl, rrl);
		return returnMetrics(rrl, null);
	}
	public static void main(String[] args) throws Exception{
		int year=2009;
		String input="E:\\ws\\ws-1\\Fusion\\2014_combsum";//Combsum2010 2010.PM2
		String qrelfile="E:\\ws\\ws-1\\Fusion\\2014.qrels-for-ndeval.txt";
		...
		n.evaluateALL(new File(input), qrelfile);
//		double[][] all=n.statisticalAll(rrl, null);
...
//		}
	 ...
	 * 
	 * @param year =2013
	 * @param input input="E:\\ws\\ws-1\\Fusion\\2013_combsum";
	 * @param qrefileqrelfile="E:\\ws\\ws-1\\Fusion\\2013.qrels-for-ndeval.txt";
	 * @return   average[2]==>>ERR-IA@20    average[11]==>>alpha-nDCG@20
	 */
	public static double[] run(int year, String input, String qrelfile) throws Exception{
		...
		ArrayList<rList> qrl=n.processQrels(qrelfile);
		n.applyQrels(qrl, rrl);	
		return n.RETURNevaluateALL(new File(input), qrelfile);
	}
}